...
- Translate `docs/09-data-model.dbml` into Drizzle schema exactly: `profiles`, `currencies`, `accounts`, `transactions`, `postings`, snake_case columns (ADR-017), FKs as specified in the DBML. (Phase 1 only created a placeholder single-table schema to prove the pipeline — this phase replaces it with the real model.)
- Decide and document the Currency-seeding flow: does creating a Profile auto-create its INR `Currency` row, or is it an explicit separate step?
...
- Implement, as plain TypeScript functions/types with no DB or UI dependency, in `src/domain/`:
  ...
  - The ownership invariant: every posting's account `profile_id` equals the transaction's `profile_id`.
...
- Repository functions wrapping Drizzle. Every Profile-scoped query takes an explicit `profileId` parameter (rule #6).
- Use-cases, each calling into Phase 3's domain functions before touching the DB: `createProfile`, `createAccount`, `archiveAccount`, `createTransaction` (atomic, validates via domain layer first), `editTransaction` (decide semantics — see open decisions below), `deleteTransaction` (hard delete, atomic cascade).
...
- Resolve the "active Profile" context question (see open decisions) before building UI on top of it.
- **Exit criteria:** a Profile, its Accounts, and a Transaction can be created end-to-end through the API layer alone.

## Phase 6 — Core UI shell

- Active-Profile switcher and context provider; empty-state onboarding.
- Accounts UI: list, create, edit, archive.
- Dashboard shell (layout only), navigation, mobile-first responsive layout.
- **Exit criteria:** a user can create a Profile and a full chart of accounts entirely through the UI.
...
- Filters exactly as scoped in `docs/08-ui-principles.md`: date, Account, classification, tag key/value (Profile scoping is implicit, not a filter). No query language, no regex — that's explicitly out of current MVP scope (see 09-ledger-gaps in the Paisa research, if it's been folded into `docs/`).
...
1. Currency-seeding flow on Profile creation (Phase 2).
...
3. Where "active Profile" context lives between requests (Phase 5) — resolved since this plan was written: real AppUser authentication (2026-08-20 delta), Profile scoped by URL segment + session ownership check, not client-held state.
