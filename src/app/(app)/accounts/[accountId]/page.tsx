...
export default async function AccountTransactionsPage(props: PageProps<"/accounts/[accountId]">) {
  ...
  const scoped = filterTransactions(listTransactions(db, profile.id), accountsById, filterState).filter(
  ...
  const baseCurrency =
    (profile.primaryCurrencyId && currenciesById.get(profile.primaryCurrencyId)) || currency;
