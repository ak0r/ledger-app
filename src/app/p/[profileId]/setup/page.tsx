...
export default async function ProfileSetupPage(props: PageProps<"/p/[profileId]/setup">) {
  const { profileId } = await props.params;
  if (listAccounts(db, profileId).length > 0) redirect(`/p/${profileId}/accounts`);
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center gap-6 p-4">
      ...
      <Card>
        ...
        <CardContent>
          <form action={createDemoProfileDataAndActivateAction.bind(null, profileId)}>
