...
// Profile-level Transactions page (docs/ledger-transaction-list-change.md),
// scoped to this Account, with a Filter drawer (description search) and
// pagination (20/50 per page, default 20) — both new here only; the
// Profile-level page stays unpaginated/unfiltered-by-description.
export default async function AccountTransactionsPage(
  props: PageProps<"/p/[profileId]/accounts/[accountId]">,
) {
  const { profileId, accountId } = await props.params;
  const searchParams = await props.searchParams;

  const account = getAccountBalances(db, profileId).find((a) => a.id === accountId);
  if (!account) notFound();

  const currency = listCurrencies(db, profileId)[0];
  ...
  const scoped = filterTransactions(listTransactions(db, profileId), accountsById, filterState).filter(
  ...
  const base = `/p/${profileId}/accounts/${accountId}`;
