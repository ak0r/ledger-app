...
export default async function AccountSettingsPage(
  props: PageProps<"/p/[profileId]/accounts/[accountId]/settings">,
) {
  const { profileId, accountId } = await props.params;

  const account = getAccount(db, accountId, profileId);
  if (!account) notFound();

  const currency = listCurrencies(db, profileId).find((c) => c.id === account.currencyId);
    ...
    <div className="flex max-w-2xl flex-col gap-4">
      ...
      {!account.isArchived && (
        <Card>
          ...
          <CardContent>
            <ArchiveAccountButton profileId={profileId} accountId={accountId} variant="outline" />
          </CardContent>
        </Card>
      )}
    </div>
