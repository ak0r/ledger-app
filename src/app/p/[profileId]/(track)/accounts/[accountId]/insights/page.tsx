...
export default async function AccountInsightsPage(
  props: PageProps<"/p/[profileId]/accounts/[accountId]/insights">,
) {
  ...
  const baseHref = `/p/${profileId}/accounts/${accountId}/insights`;
  ...
  const summary = getAccountRangeSummary(db, profileId, accountId, range);

  const balanceTrendData = () =>
    getBalanceTrend(db, profileId, accountId, range).map((point) => ({
      date: point.date,
      balance: fromMinorUnits(point.balance, scale),
    }));

  const cashflowData = () =>
    getMonthlyCashflow(db, profileId, accountId, range).map((point) => ({
      month: point.month,
  ...
  switch (account.instrumentType) {
    ...
    case "INCOME":
    case "EXPENSE": {
      const monthly = getMonthlyCashflow(db, profileId, accountId, range);
