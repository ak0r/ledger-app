...
export function commitImport(db: Db, input: CommitImportInput): ImportFileRow[] {
  ...
  return db.transaction((tx) => {
        ...
        allPostingRows.push(
          {
            ...
            debit: amounts.known.debit,
            credit: amounts.known.credit,
            createdAt: now,
            updatedAt: now,
          },
          {
            ...
            debit: amounts.counter.debit,
            credit: amounts.counter.credit,
            createdAt: now,
            updatedAt: now,
          },
        );
    ...
    for (let i = 0; i < allTransactionRows.length; i++) {
      const postingsForTransaction = allPostingRows.slice(i * 2, i * 2 + 2);
      const violations = validateTransaction(
        { profileId: input.profileId, postings: postingsForTransaction },
        accountRefs,
      );
