...
describe("createMemberCore", () => {
  it("creates a Member from valid input", () => {
    const db = createTestDb();
    const result = createMemberCore(db, { name: "Amit" });
  ...
  it("rejects a blank name with fast client feedback, no DB write", () => {
    const db = createTestDb();
    const result = createMemberCore(db, { name: "  " });
  ...
  it("rejects malformed input without throwing", () => {
    const db = createTestDb();
    const result = createMemberCore(db, { nope: true });
    expect(result.success).toBe(false);
  });
});
